...
5. `QUANTIZATION`: (Optional) Whether to use AWQ quantization or not. Specify with "awq" if you are using a AWQ quant model.
...
#### Mistral 7B AWQ Instruct:
`docker build . --platform linux/amd64 --build-arg HUGGING_FACE_HUB_TOKEN=your_hugging_face_token_here --build-arg MODEL_NAME=TheBloke/Mistral-7B-Instruct-v0.1-AWQ --build-arg TOKENIZER=hf-internal-testing/llama-tokenizer --build-arg STREAMING=True --build-arg QUANTIZATION=awq`
